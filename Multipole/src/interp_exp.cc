...
void Multipole_Interp(CCTK_ARGUMENTS, CCTK_REAL xs[], CCTK_REAL ys[],
  ...
  const CCTK_INT num_points = CCTK_MyProc(cctkGH) == 0
                                  ? (ntheta + 1) * (nphi + 1)
                                  : 0; // Only the 0 processor needs the points

  int ierr = -1;

  const void *interp_coords[num_dims] = {(const void *)xs, (const void *)ys,
                                         (const void *)zs};
  ...
  if ((ierr = Util_TableSetIntArray(param_table_handle, num_points,
  ...
  //     param_table_handle,
  //     coord_system_handle,
  //     CCTK_MyProc(cctkGH) == 0 ? (ntheta+1)*(nphi+1) : 0, // Only the 0
  //     processor needs the points CCTK_VARIABLE_REAL, interp_coords,
  //     num_input_arrays,
  //     input_array_indices,
  //     num_output_arrays,
  //     output_array_types,
  ...
  ierr = DriverInterpolate(
      ...
      coord_system_handle, num_points, CCTK_VARIABLE_REAL, interp_coords,
      num_input_arrays, (int const *const)input_array_indices,
