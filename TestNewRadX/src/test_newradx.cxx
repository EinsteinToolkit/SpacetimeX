...
constexpr T Laplacian_4thorder(const Loop::GF3D2<T> gf,
  ...
  return 1.0 / 12.0 *
         ((-gf(ip2jk) + 16 * gf(ip1jk) - 30 * gf(ijk) - gf(im2jk) +
           16 * gf(im1jk)) /
              (p.DX[0] * p.DX[0]) +
          (-gf(ijp2k) + 16 * gf(ijp1k) - 30 * gf(ijk) - gf(ijm2k) +
           16 * gf(ijm1k)) /
              (p.DX[1] * p.DX[1]) +
          (-gf(ijkp2) + 16 * gf(ijkp1) - 30 * gf(ijk) - gf(ijkm2) +
           16 * gf(ijkm1)) /
              (p.DX[2] * p.DX[2]));
}
...
template <typename T>
constexpr void gaussian(const T t, const T A, const T W, const T x, const T y,
                        const T z, T &uu, T &vv) {
...
// Scheduled functions
extern "C" void TestNewRadX_Init(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTSX_TestNewRadX_Init;
  DECLARE_CCTK_PARAMETERS;

  grid.loop_int_device<0, 0, 0>(
      grid.nghostzones,
      [=] CCTK_DEVICE(const Loop::PointDesc &p) CCTK_ATTRIBUTE_ALWAYS_INLINE {
        gaussian(cctk_time, gaussian_a0, gaussian_w0, p.x - gaussian_x0,
                 p.y - gaussian_y0, p.z - gaussian_z0, uu(p.I), vv(p.I));
      });
}
...
extern "C" void TestNewRadX_CompareSolution(CCTK_ARGUMENTS) {
  ...
  grid.loop_int_device<0, 0, 0>(
      ...
      [=] CCTK_DEVICE(const Loop::PointDesc &p) CCTK_ATTRIBUTE_ALWAYS_INLINE {
        ...
        gaussian(cctk_time, gaussian_a0, gaussian_w0, p.x - gaussian_x0,
                 p.y - gaussian_y0, p.z - gaussian_z0, u_analytic, v_analytic);
        uu_err(p.I) = uu(p.I) - u_analytic;
        vv_err(p.I) = vv(p.I) - v_analytic;
      });
}

extern "C" void TestNewRadX_EstimateError(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTSX_TestNewRadX_EstimateError;
  DECLARE_CCTK_PARAMETERS;
