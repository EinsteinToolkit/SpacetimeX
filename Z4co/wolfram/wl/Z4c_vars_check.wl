...
dtEvolVarlist = DefTensors[{dtchi[], PrintAs -> "\!\(\*SubscriptBox[\(\[PartialD]\), \(t\)]\)\[Chi]"
...
EvolVarlist = DefTensors[{chi[], PrintAs -> "\[Chi]"}, {gamt[-i, -j], Symmetric[
  {-i, -j}], PrintAs -> "\!\(\*OverscriptBox[\(\[Gamma]\), \(~\)]\)"}, {exKh[
...
dEvolVarlist = DefTensors[{dchi[-k], PrintAs -> "\[PartialD]\[Chi]"}, {
...
ddEvolVarlist = DefTensors[{ddchi[-i, -j], Symmetric[{-i, -j}], PrintAs
...
TmunuVarlist = DefTensors[{eTtt[], PrintAs -> "\!\(\*SubscriptBox[\(T\), \(tt\)]\)"
...
IntermediateVarlist = DefTensors[{detinvgamt[], PrintAs -> "1/\!\(\*OverscriptBox[\(\[Gamma]\), \(~\)]\)"
...
DDVarlist = DefTensors[{tDtDchi[-i, -j], Symmetric[{-i, -j}], PrintAs ->
   "\!\(\*OverscriptBox[\(D\), \(~\)]\)\!\(\*OverscriptBox[\(D\), \(~\)]\)\[Chi]"
  }, {DDalpha[-i, -j], Symmetric[{-i, -j}], PrintAs -> "DD\[Alpha]"}];

RVarlist = DefTensors[{Rtchi[-i, -j], Symmetric[{-i, -j}], PrintAs -> "\!\(\*SuperscriptBox[OverscriptBox[\(R\), \(~\)], \(\[Chi]\)]\)"
  }, {Rt[-i, -j], Symmetric[{-i, -j}], PrintAs -> "\!\(\*OverscriptBox[\(R\), \(~\)]\)"
  }, {R[-i, -j], Symmetric[{-i, -j}]}, {trR[], PrintAs -> "R"}];

(* Matter *)

MatterVarlist = DefTensors[{rho[], PrintAs -> "\[Rho]"}, {Sm[-i], PrintAs
   -> "S"}, {Ss[-i, -j], Symmetric[{-i, -j}], PrintAs -> "S"}, {trSs[], PrintAs
   -> "S"}];

(* Constraints *)

dAtUUVarlist = DefTensors[{trdexAtUU[i], PrintAs -> "\[PartialD]\!\(\*OverscriptBox[\(A\), \(~\)]\)"
  }];

ConstraintVarlist = DefTensors[{ZtC[i], PrintAs -> "\!\(\*OverscriptBox[\(Z\), \(~\)]\)"
