...
void SphericalSurface_Set (CCTK_ARGUMENTS)
{
  ...
  for (n=0; n<nsurfaces; ++n) {
    ...
    if (set_spherical[n]) {
      ...
      /* This surface is not set here -- another thorn (e.g. AHFinderDirect)
         will populate it later. schedule.ccl nevertheless declares sf_active,
         sf_valid, sf_info, sf_origin and sf_radius as written everywhere, so
         they must be given a defined value: CarpetX's validity checking sees
         the declared WRITES, finds NaNs, and aborts the run. Mark the surface
         inactive and invalid, which is what consumers should honour. */
      ...
      sf_centroid_y[n] = 0;
      sf_centroid_z[n] = 0;
      sf_quadrupole_xx[n] = 0;
      sf_quadrupole_xy[n] = 0;
      sf_quadrupole_xz[n] = 0;
      sf_quadrupole_yy[n] = 0;
      sf_quadrupole_yz[n] = 0;
      sf_quadrupole_zz[n] = 0;
      sf_min_radius[n] = 0;
      sf_max_radius[n] = 0;
      sf_min_x[n] = 0;
      sf_min_y[n] = 0;
