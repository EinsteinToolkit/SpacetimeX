...
Therefore, the boundary conditions are effectively applied on a smaller,
artificial boundary that now reside within what \code{CarpetX} considers
as the interior.

It is necessary to introduce this change because


Currently, this thorn only supports single-patch, Cartesian grids provided by
\code{CarpetX},
but not multi-patch coordinate systems.
...
\code{AsterX}\cite{SpacetimeX_NewRadX_AsterX}, and
\code{Z4c}\cite{SpacetimeX_NewRadX_Z4c}.
...
\bibitem{SpacetimeX_NewRadX_AsterX} ``AsterX'',
