...
SetOutputFile[FileNameJoin[{Directory[], "Z4cow_set_rhs_freezeTheta.hxx"}]];
...
$MainPrint[] :=
  Module[{},
    ...
    pr[];
    PrintEquations[{Mode -> "Main"}, Drop[dtEvolVarlist, {-3}]];
    pr[];
    PrintEquations[{Mode -> "Main", SuffixName -> "freezeTheta"}, Take[dtEvolVarlist, {-3}]];
    pr[];
    pr["  });"];
    pr["});"];
  ];
